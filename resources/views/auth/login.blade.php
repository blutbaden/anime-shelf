...
<div class="w-full max-w-md">
    ...
    <div class="text-center mb-8">
        ...
        <p class="mt-2 text-gray-500 dark:text-gray-400 text-sm">{{ __('Sign in to your account') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="remember" class="w-4 h-4 rounded border-gray-300 text-anime-600 focus:ring-anime-500">
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Remember me') }}</span>
                </label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm text-anime-600 hover:text-anime-700 dark:text-anime-400">{{ __('Forgot password?') }}</a>
                @endif
            </div>
        ...
        <p class="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('register') }}" class="text-anime-600 hover:text-anime-700 dark:text-anime-400 font-medium">{{ __('Sign up') }}</a>
        </p>
    </div>
</div>
